...
> **My super mini robotic arm project.**
>
> Video introduction: [【Homemade】I made a robotic arm of Iron Man! 【Hardcore】](https://www.bilibili.com/video/BV12341117rG)
>
...
* 3D model design source files added.
* Gripper hardware design files and LED ring PCB added
* Wireless spatial positioning controller PCB added
* Wireless teaching device Peak software and hardware project added (as submodule)
* REF hardware design files added
* DummyStudio PC software added
* Dummy core controller firmware source code added (see later for usage instruction)
* 42 stepper motor driver hardware project added
* 20 stepper motor driver hardware project added
* Firmware source code for 42/20 stepper motor drivers added
* Command line debugging tool reftool added (based on the odrivetool framework)
* Portable suitcase model files added



> This is the complete design of the original robotic arm in the video, which is costly and difficult to produce. Therefore, students who want to replicate it are advised to wait for the **Dummy Youth Version** I will release later, which will have the following improvements：
>
> 1. The whole machine is redesigned, using 3D printing as the manufacturing solution (the original version used aluminum CNC), significantly reducing manufacturing costs.
> 2. My own designed small cycloidal pinwheel reducer replaces the original harmonic reducer, greatly reducing part costs.
> 3. All software and firmware are compatible with the original version, with identical functionality. 
> 4. Added my own designed PC software and mobile APP (striving to include user initialization setup guide).
> 5. Improved the wiring method of the original motor driver, which used soldering for power wiring, inconvenient for installation and disassembly. The youth version will use 4P connectors (power + CAN bus) for connection.
> 6. Strive to keep the total cost under 2000 RMB (~= $300 USD).
> 7. **Most importantly, a nanny-level video tutorial will be provided!**



## About Structural Design

The original design in my video used `stepper  motors` + Harmonic's `harmonic reduction modules`, the latter being costly (I bought a second-hand one for about 600 RMB each). Therefore, to allow everyone to replicate this project as much as possible, I will add a low-cost solution of `homemade cycloidal pinwheel reducer` + `3D printing` later. 

> The cycloidal reducer has been designed and is currently being verified. It is expected to be made by cutting PC (or acrylic) combined with 3D printing. The precision will decrease, but the functionality remains unchanged. The total hardware cost is hoped to be controlled within 2000 RMB (~= $300 USD).
...
To achieve the main robotic arm motion control functions, the core consists of four boards:

* REF core board
* REF baseboard (i.e., the controller circuit board inside the base of the robotic arm)
* Stepper motor driver
* Peak teaching device
